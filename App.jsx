    ...
    adminTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        if (import.meta.env.VITE_ADMIN_PASS_HASH) {
          ...
          const isValid = timingSafeEqual(hashHex, String(import.meta.env.VITE_ADMIN_PASS_HASH));
          setIsAdminAuth(isValid);

          // Security: Clear plaintext password from state immediately upon successful authentication
          if (isValid) {
            setAdminPass("");
          }
        } else {
