...
## 2026-04-01 - Progressive Disclosure for Strict Inputs
**Learning:** Users can be confused by strict input constraints or hidden shortcuts if they are not explicitly communicated. Static helper text can clutter the UI.
**Action:** For strict form inputs with hidden keyboard shortcuts, use dynamic `aria-live="polite"` helper text bound to the input via `aria-describedby` to progressively explain constraints and reveal shortcuts.
