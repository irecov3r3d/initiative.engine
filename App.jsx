...
// Performance: Pre-compute a lookup table for high-speed ArrayBuffer to hex string conversion.
// Expected Impact: Eliminates per-byte .toString(16) and .padStart() overhead, significantly reducing execution time and memory allocation during cryptographic operations.
    ...
    adminTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        if (import.meta.env.VITE_ADMIN_PASS_HASH) {
          ...
          let hashHex = '';
          // Performance: Replaced inline .toString(16) and .padStart() with pre-computed lookup table
          // Expected Impact: O(1) string retrieval per byte eliminates string formatting overhead during tight loops
